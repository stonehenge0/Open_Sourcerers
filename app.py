...
def pad_result(result: dict):
    ...
    if result_padded['efficiency'] == 1:
        ...
        result_padded['e_text'] = 'The charity evaluator ' + {{ result_padded['evaluator'] }} + ' sees potential for impact in ' + {{ result_padded.name }} + '. However, they need additional evidence or research for a better evaluation. This might change with a future evaluation.'
    elif result_padded['efficiency'] == 3:
        result_padded['e_title'] = 'Promising Impact'
        result_padded['e_text'] = 'According to' + {{ result_padded['evaluator'] }} + ' ' + {{ result_padded['name'] }} + ' demonstrates potential effectiveness, but does not belong to the top-rated. ' + {{ result_padded['name'] }} + ' is regularly in evaluated and improved.' 
    elif result_padded['efficiency'] == 4:
        result_padded['e_title'] = 'Top-rated Impact'
        result_padded['e_text'] = {{ result_padded['name'] }} + ' is recognized as highly effective and impactful by ' + {{ result_padded['evaluator'] }}
...
def result_to_html(data, result_padded):
    with open(os.path.join(os.getcwd(),'docs','result.html'), 'w') as f:
        data = data
        print(data)
        ...
        RESULT_HTML = f""" <!DOCTYPE html>
            ...
            <div class="w3-container w3-padding-15" id="about">
                ...
                <div class="w3-display-container">
                    ...
                    <p>More info on the efficiency and cost <a href={ result_padded['link_cost'] }>here</a></p>
                    <p>Does { result_padded['name'] } work to prevent an existential crisis for humanity like clmate change? { result_padded['xcrisis'] }</p>
                    <br>
                    <p>More info about the charity <a href={ result_padded['link_website'] }>here</a></p>
                    <br>
                ...
                <div class="w3-half l3 m6 w3-margin-bottom">
                <div class="w3-display-container">
                    <h3>Some dynamic visualization here</h3>
                    <img src="static/Apfelkuchen.jpg" alt="Apfelkuchen" style="width:100%" title="Placeholder Apfelkuchen">
                    <p>This text will be replaced with a description of the best fit charity.</p>
                </div>
                </div>
...
def submit_questionnaire():
    global result
    global result_padded
    if request.method == 'POST':
        data=request.form.to_dict()
        result = analysis.produce_result()
        result_padded = pad_result(result)
